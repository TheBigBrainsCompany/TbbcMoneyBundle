...
final class MoneyTypeTest extends TestCase
{
    ...
    public function testGetSqlDeclaration(): void
    {
        ...
        $this->assertSame('varchar(255)', $this->type->getSqlDeclaration(['varchar'], $this->platform));
    }
    ...
    {
        $this->assertNotInstanceOf(Money::class, $this->type->convertToPHPValue(null, $this->platform));
    }
    ...
    public function testConvertToPHPValueValue(): void
    {
        ...
        $this->assertSame($money->getAmount(), $this->type->convertToPHPValue($value, $this->platform)->getAmount());

        $this->assertSame($money->getCurrency()->getCode(), $this->type->convertToPHPValue($value, $this->platform)->getCurrency()->getCode());
    }
    ...
    public function testConvertToDatabaseValueMoney(): void
    {
        $money = new Money(100, new Currency('EUR'));
        $value = $money->getCurrency()->getCode() . ' ' . $money->getAmount();
        $this->assertSame($value, $this->type->convertToDatabaseValue($money, $this->platform));
    }
    ...
    {
        $this->assertSame('money', $this->type->getName());
    }
}
